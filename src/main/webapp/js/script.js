...
// attach events to filters to re-run filterProducts
document.addEventListener('change', (e)=>{
  // when a price radio changes, also reset slider to max default to avoid conflict
  if(e.target && e.target.name === 'price'){
    // if user chose a radio, reset slider to default max
    if(priceRange) priceRange.value = priceRange.max;
    filterProducts();
  }

  // when pettype checkbox changed -> filter
  if(e.target && e.target.classList && e.target.classList.contains('pettype')){
    filterProducts();
  }

  // when ptype radio in subtabs changed -> filter
  if(e.target && (e.target.name === 'dog-ptype' || e.target.name === 'cat-ptype')){
    filterProducts();
  }
});
